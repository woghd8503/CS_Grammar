...
// 우리는 똑같은 일을 보통의 클래스를 통해서도 할 수 있습니다. 그냥 메소드를 선언한 다음, 클래스에 대한 메뉴얼을 작성해서 코드와 함께 배포합니다. "이 클래스는 직접
// 인스턴스화하지 말고 파생 클래스를 만들어 사용하세요. 아, 그리고 MethodA() MethodB()를 꼭 오버라이딩해야 합니다." 라는 식으로요. 그러나 이를 프로그래머들이 준수하도록
// 강제시킬 없음은 물론입니다.

// 하지만, 추상 클래스를 이용한다면 이러한 설명이 필요 없습니다. 추상 클래스와 추상 메소드 자체가 이런 설명을 담고 있는 것이니까요. 혹시 내가 만든 추상 클래스를
// 이용하는 다른 프로그래머가 파생 클래스를 만들어야 하며 모든 추상 메소드를 구현해야 한다는 사실을 잊어버린다 해도, 컴파일러가 상기시켜줄 겁입니다. 이것이 우리가
// 추상 클래스를 사용하는 이유입니다.
